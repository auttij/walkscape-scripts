from file_utils import read_json, json_files, get_localization
from jsondiff import diff
import filecmp

class DiffTool:
  def __init__(self, src_1, src_2, excluded_keys=None) -> None:
    self.src_1 = src_1
    self.src_2 = src_2
    self.excluded = excluded_keys if excluded_keys else []
    self.diff = {}
    self.create_diff_structure()
    self.generate_diffs()

  def create_diff_structure(self):
    def trim_filename(filename):
      return filename.split('/')[-1].split('\\')[-1].split('.')[0]
    
    relatative_files_1 = json_files(self.src_1, True)
    relatative_files_2 = json_files(self.src_2, True)
    in_both = [trim_filename(i) for i in relatative_files_1 if i in relatative_files_2]

    absolute_files_1 = json_files(self.src_1)
    absolute_files_2 = json_files(self.src_2)

    all = {
      e: { 'path_1': absolute_files_1[i], 'path_2': absolute_files_2[i] } for i, e in enumerate(in_both)
    }
    not_excluded = { k: v for k, v in all.items() if k not in self.excluded }
    self.diff = not_excluded

  def file_diff(self, key):
    ...
    base_diff = diff(data_1, data_2, syntax='symmetric')
...
def main():
  path_0 = "C:\\Games\\Walkscape\\0.2.0-beta+322 data"
  path_1 = "C:\\Games\\Walkscape\\0.2.0-beta+323\\data\\flutter_assets\\assets\\data"
  path_2 = "C:\\Games\\Walkscape\\0.2.0-beta+324\\data\\flutter_assets\\assets\\data"
  ...
  d = DiffTool(path_0, path_1, excluded_keys=excluded)
  d.pretty_print()
